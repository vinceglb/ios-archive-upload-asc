...
REPO="vinceglb/ios-archive-upload-asc"
INSTALL_DIR="${HOME}/.local/bin"
VERSION="${IOS_GHA_SETUP_VERSION:-latest}"
ASSET_NAME="ios-gha-setup.sh"
CHECKSUM_NAME="ios-gha-setup.sh.sha256"
...
if [[ $# -gt 0 ]]; then
  case "$1" in
    ...
    -h|--help)
      cat <<USAGE
Usage: install-ios-gha-setup.sh [--version <latest|tag>]

Environment variables:
  IOS_GHA_SETUP_VERSION   Version tag to install (default: latest)
USAGE
      exit 0
      ;;
    *)
...
mkdir -p "${INSTALL_DIR}"
install -m 0755 "${ASSET_PATH}" "${INSTALL_DIR}/ios-gha-setup"

echo "Installed ios-gha-setup ${RESOLVED_TAG} to ${INSTALL_DIR}/ios-gha-setup"

case ":${PATH}:" in
  *":${INSTALL_DIR}:"*)
    echo "Run: ios-gha-setup wizard"
    ;;
  *)
    echo "${INSTALL_DIR} is not in PATH. Add this line to your shell profile:"
    echo "  export PATH=\"${INSTALL_DIR}:\$PATH\""
    echo "Then run: ios-gha-setup wizard"
    ;;
esac
