...
`ios-gha-setup` is a guided bootstrap tool for iOS GitHub Actions with `ios-archive-upload-asc`.
...
```text
~/.local/state/ios-gha-setup/session.json
```
...
```bash
ios-gha-setup check --repo owner/repo --write-workflows --repo-dir /path/to/repo
```
